...
public:
	...
	static ImageDataClassSafe* GetImageDataFromServer(ppmfc::CString ID, ppmfc::CString name);
	...
	static HANDLE hRequestEvent;
	static HANDLE hResponseEvent;
	...
	static bool SendIniMapToServer(int type = -1);
	static bool SendImageToServer(const ppmfc::CString& imageID, const ImageDataClassSafe* imageData);
	static bool RequestImageFromServer(const ppmfc::CString& ID, const ppmfc::CString& imageID, ImageDataClassSafe& outImageData);
	static void SendRequestText(const char* text);
	static void SendRequestText(std::vector<char>& text);
	static void SendPackedText(const std::string& header, const std::string& text);
	static HANDLE CreateDataPipe();
};