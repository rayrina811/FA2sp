...
static void GetTileScaledSize(CTileSetBrowserView* pThis, int tileIndex, int& w, int& h)
{
    if (pThis->CurrentTileset < 10000)
    {
        ...
        w = CTileSetBrowserViewExt::GetAddedWidth(tileIndex)
            + std::max(tile.Bounds.right - tile.Bounds.left, 60l);
        h = CTileSetBrowserViewExt::GetAddedHeight(tileIndex)
            + std::max(tile.Bounds.bottom - tile.Bounds.top, 30l);
    }
    else
    {
        auto tileData = CMapDataExt::GetCustomTile(tileIndex);
        CTileSetBrowserViewExt::GetCustomTileSize(tileData, w, h);
    }
...
DEFINE_HOOK(4F36A0, CTileSetBrowserView_RenderTile, 5)
{
    ...
    R->EAX(CTileSetBrowserViewExt::RenderTile(iTileIndex));
    return 0x4F3BEF;
...
DEFINE_HOOK(4F2B10, CTileSetBrowserView_SetTileSet, 7)
{
    ...
    if (dwTileSet < 10000)
    {
        ...
        for (int i = 0; i < tileCount; i++)
        {
            ...
            {
                int width = CTileSetBrowserViewExt::GetAddedWidth(tileIndex) 
                    + std::max(CMapDataExt::TileData[tileIndex].Bounds.right
                    - CMapDataExt::TileData[tileIndex].Bounds.left, 60l);
                int height = CTileSetBrowserViewExt::GetAddedHeight(tileIndex)
        ...
        for (int i = 0; i < tileCount; i++)
        {
            pThis->TileSurfaces[i] = CTileSetBrowserViewExt::RenderTile(tileStart + i);
    ...
    else
    {
        ...
        for (int i = 0; i < tileCount; i++)
        {
            pThis->TileSurfaces[i] = CTileSetBrowserViewExt::RenderTile(CMapDataExt::GetCustomTileIndex(dwTileSet, i));
            CIsoViewExt::ScaleSurface(&pThis->TileSurfaces[i], CTileSetBrowserFrameExt::TileSetBrowserViewScaledFactor);
        }
    }
    ...
    if (pThis->CurrentMode == 1)
    {
		...
		for (int i = 0; i < pThis->TileSurfacesCount; i++)
        {
            ...
            if (pThis->CurrentTileset < 10000)
            {
				...
				curwidth = CTileSetBrowserViewExt::GetAddedWidth(tileIndex) +
						    std::max(tile.Bounds.right 
                            - tile.Bounds.left, 60l);
				curheight = CTileSetBrowserViewExt::GetAddedHeight(tileIndex) + 
            ...
            else
            {
                ...
                CTileSetBrowserViewExt::GetCustomTileSize(tileData, curwidth, curheight);
            }
...
DEFINE_HOOK(4F4650, CTileSetBrowserView_GetAddedHeight, 9)
{
    ...
    R->EAX(CTileSetBrowserViewExt::GetAddedHeight(iTileIndex));
