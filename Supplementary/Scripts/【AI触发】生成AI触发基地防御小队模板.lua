...
local dlg = LuaDialog:new("基地防御小队生成器", false, 580, 350)
...
-- 创建脚本（11-11：进入原地警戒状态）
local s = script:new()
s.name = "<" .. base_name .. " DEF>"
s:add_action(11, 11)
s:apply()

-- 创建小队
local t = team:new()
t.name = "<" .. base_name .. " DEF>"
...
print("  √ 增援部队")
print("  √ 小队成员可被重组")
print("脚本行为：11-11（进入原地警戒状态）")
print("AI触发：已启用，阵营=" .. tostring(side))
print("=======================")

message_box("基地防御小队已成功生成！\n\n特遣部队: " .. task.name .. "\n脚本: " .. s.name .. "\n小队: " .. t.name .. "\nAI触发: " .. ai.name, "执行成功", 1)
